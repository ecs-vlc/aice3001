...
\begin{question}{20}
  \begin{qparts}
      ...
      \qpart[3]{Show that if the columns, $\bm{m}_j$, of an $n\times n$ matrix,
        ...
        \begin{align*}
          \forall_{i=1}^n: \quad \sum_{i=1}^n c_j\, M_{ij} = 0
        \end{align*} 
      \begin{answer}
      ...
      \qpart[2]{For finite-dimensional square matrices if
        ...
        Prove that if $\mat{A}\mat{B}= \Id$ and
...
\begin{question}{20}
  ...
  \begin{align*}
    \mathrm{tr}\bra{\mat{A}} &= \mathrm{tr}\bra{\mat{A}^\tr}
    & \mathrm{tr}\bra{\mat{A}\,\mat{B}} &= \mathrm{tr}\bra{\mat{B}\,\mat{A}} .
  \end{align*}
  ...
  \begin{qparts}
    ...
    \qpart[5]{For a vector $\bm{a}\in\mathbb{R}^m$, a vector
      ...
      \begin{align*}
        \bm{a}^\tr\, \mat{U}\, \bm{b}
        = \mathrm{tr}\bra{\strut \mat{U}^\tr\, \bm{a}\, \bm{b}^\tr}.
      \end{align*}
      The quickest route is to write both sides out in components.  Note
      that $\bm{a}\,\bm{b}^\tr$ is an $m\times n$ matrix while
      $\bm{a}^\tr\,\mat{U}\,\bm{b}$ is a scalar.
      \begin{answer}
        Writing the left-hand side in components
        \begin{align*}
          \bm{a}^\tr\, \mat{U}\, \bm{b}
          = \sum_{i=1}^m \sum_{j=1}^n a_i\, U_{ij}\, b_j .
        \end{align*}
        For the right-hand side, the outer product $\bm{a}\,\bm{b}^\tr$
        has elements $[\bm{a}\,\bm{b}^\tr]_{ik} = a_i\, b_k$, so
        \begin{align*}
          \bigl[\mat{U}^\tr\, \bm{a}\, \bm{b}^\tr\bigr]_{jk}
          = \sum_{i=1}^m [\mat{U}^\tr]_{ji}\, [\bm{a}\,\bm{b}^\tr]_{ik}
          = \sum_{i=1}^m U_{ij}\, a_i\, b_k .
        \end{align*}
        The trace is the sum of the diagonal elements, $k=j$, so
        \begin{align*}
          \mathrm{tr}\bra{\strut \mat{U}^\tr\, \bm{a}\, \bm{b}^\tr}
          = \sum_{j=1}^n \sum_{i=1}^m U_{ij}\, a_i\, b_j
          = \sum_{i=1}^m \sum_{j=1}^n a_i\, U_{ij}\, b_j ,
        \end{align*}
        which is the left-hand side.

        Alternatively, $\bm{a}^\tr\,\mat{U}\,\bm{b}$ is a scalar, so it
        equals its own trace, and using
        $\mathrm{tr}(\mat{A}\,\mat{B}) =
        \mathrm{tr}(\mat{B}\,\mat{A})$ together with
        $\mathrm{tr}(\mat{A}) = \mathrm{tr}(\mat{A}^\tr)$
    ...
    \qpart[6]{Use this to find the derivative of
      ...
      \begin{answer}
        ...
        \begin{align*}
          \bm{c}^\tr\, \mat{U}\, \bm{d}
          = \mathrm{tr}\bra{\strut \mat{U}^\tr\, \bm{c}\, \bm{d}^\tr} .
        \end{align*}
    ...
    \qpart[5]{Using the results
      ...
      \begin{answer}
        ...
        \begin{align*}
          ...
          O(\epsilon^2)  \\
          &= \epsilon\, \textrm{tr} \mat{U}^\tr (\mat{X}^{-1})^\tr
        \end{align*}
        Thus,
        \begin{align*}
...
\begin{question}{20}
    ...
    to be accurate up to the numerically precision of your machine
    (typically around $10^{-16}$).  For python the following my be
    useful
   ...
   \qpart[2]{Compute $\mat{D}= \mat{A}\, \mat{B}$ and $\mat{E} =
    ...
    \qpart[2]{
      Compute (a) $\mathrm{tr} \mat{A}\, \mat{B}$, (b)
      $\bra{\mathrm{tr} \mat{A}}\bra{\mathrm{tr} \mat{B}}$ (c)
        $\mathrm{tr} \mat{B}\, \mat{A}$ and (d) $\mathrm{tr}
    ...
    \qpart[10]{
      Although, we write can define the gradient with respect to a
      matrix using
      \begin{align*}
      \lim_{\epsilon\rightarrow 0}  \frac{f(\mat{A} +
        \epsilon\,\mat{U}) - f(\mat{A})}{\epsilon} = \mat{U}^\tr
        \frac{\partial f(\mat{A})}{\partial \mat{A}}
      \end{align*}
      numerically the converges is slow.  This is problematic as $f(\mat{A} +
      ...
      \begin{align*}
        \mat{U}^\tr \frac{\partial f(\mat{A})}{\partial \mat{A}}
      \end{align*}
      numerically.  Compare this to $\mat{U}^\tr (\mat{X}^{-1})^\tr$
      \begin{answer}
        Use the following
\begin{alltt}
eps = 0.0001\\
