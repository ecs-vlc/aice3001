...
#+SUBTITLE: Subsidiary Notes --- Lecture 19: Probability
#+SETUPFILE: setup.org
...
** Basic Language
   ...
   - The elementary events are /mutual exclusive/ so that if
     $\omega_{i}, \omega_{j} \in \Omega$ then $\omega_{i} \cap \omega_{j} = \emptyset$ (this just means there is no intersection
   ...
   - Now we are often not interested in elementary events (they may be
     too small)
   - Instead we consider *events* $\mathcal{E}= \bigcup\limits_{i\in\mathcal{I}}\omega_i$ where $\mathcal{I}$ is
     some /index set/
...
** Probability and Random Variables
   - We can attribute a *probability*,  $\Prob{\mathcal{E}}$, to an event,
    $\mathcal{E}$
   - Probabilities are number between 0 and 1: $0\leq \Prob{\mathcal{E}} \leq 1$
...
** Expectations
  ...
  - The expectation of a constant $c$ is $c$
    \begin{align*}
      ...
      \end{cases}
    \end{align*}
  - A consequence of this is that $\av[X]{\av[X]{g(X)}} = \av[X]{g(X)}$
...
** Calculus of Probabilities
  ...
  - *Conditional Probability*
    ...
    - We might believe there is a causal relationship between getting
      very cold and catching a cold $\Prob{\text{Catch cold} \mid \text{freezing}} > \Prob{\text{Catch cold}}$
  ...
  - *Independence*
    - Random variables $X$ and $Y$ are said to be \emph{independent} if
      ...
      \end{align*}
    - Because $\Prob{X,Y}= \Prob{X|Y}\,\Prob{Y}$ and $\Prob{X,Y}= \Prob{Y|X}\,\Prob{X}$ independence implies
      \begin{align*}
...
* Exercises
  1. Suppose I make an investment of capital $C_{0}$ and each day I
     get a random return $R_{t}$ such that $C_{t} = (1+r_{t})\,C_{t-1}$. Taking logs
     $$ \logg{C_{t}} = \log(1 + R_{t}) + \log{C_{t-1}} \approx
     R_{r}+\logg{C_{t-1}}$$
     where we assume $|R_{t}| \ll 1$.  Rearranging $\logg{C_{t}} - \logg{C_{t-1}} \approx R_{t}$ and summing
     \begin{align*}
     \sum_{t=1}^{T} \bra{\logg{C_{t}} - \logg{C_{t-1}}} =
     \logg{C_{T}} - \logg{C_{0}} = \sum_{{t=1}}^{T} R_{t}
     \end{align*}
     Now $\logg{C_{T}} - \logg{C_{0}} = \logg{C_{T}/C_{0}} = \logg{G_{T}}$ where $G_{T}=C_{T}/C_{0}$ is the multiplicative
     gain (or loss) in the investment. Let $L_{T} = \logg{G_{T}}$.
     Assuming we live in a random world so that $R_{t} \sim \mathcal{N}(0,\epsilon)$. That is, the return each day is
     ...
     That is, $f_{L}(\ell) = \mathcal{N}(\ell\mid 0, \sqrt{T}\,\epsilon)$. Compute the PDF of $G_{T}$ using
...
* COMMENT [[file:wasserstein.org][Previous]]  [[file:experimentalDesign.org][Next]]
