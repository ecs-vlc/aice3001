...
#+SUBTITLE: Subsidiary Notes --- Lecture 11: Stochastic Gradient Descent
#+SETUPFILE: setup.org
...
** Adaptive Methods
   ...
   * *AdaDelta*
     * AdaDelta is an algorithm  that estimates the curvature by computing
       ...
       \begin{align*}
          S^{g,(t+1)}_i = (1-\gamma) S^{g,(t)}_i + \gamma \left(
          \frac{\partial L_{\mathcal{B}}(\bm{w}^{(t)})}{\partial w_i^{(t)}} \right)^2
       \end{align*}
       * This is a running average (it slowly forgets the past)
     * We also computes a running average of the squared weight
	\begin{align*}
          S^{w,(t+1)}_i = (1-\gamma) S^{w,(t)}_i + \gamma  \, (w_i^{(t)})^2
	\end{align*}
     * It then updates each weight according to
       \begin{align*}
	 w_i^{(t+1)} = w_i^{(t)} - \eta \,
         \sqrt{\frac{S^{w,(t+1)}_i+\epsilon}{S^{g,(t+1)}_i+\epsilon}}\,
...
* COMMENT [[file:optimisation.org][Previous]]  [[file:constrainedOpt.org][Next]]
