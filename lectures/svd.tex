%% Standalone lecture -- build with:  pdflatex -shell-escape svd
\input{preamble}
...
\begin{slide}
\section{Singular Valued Decomposition}

\begin{PauseHighLight}
  \begin{itemize}
  \item One of the most immportant results in linear algebra is that
    any matrix $\mat{X}$ can be decomposed as
    \begin{align*}
      \mat{X} = \mat{U} \mat{S} \mat{V}^\tr\pause
    \end{align*}
  \item If $\mat{X}$ is a $m\times n$ matrix then $\mat{U}$ is a
    $m\times m$ orthogonal matrix and $\mat{V}$ is an $n\times n$
    orthogonal matrix\pause{} i.e. $\mat{U}\mat{U}^\tr =
    \mat{U}^\tr\mat{U} = \mat{I}, \; \mat{V}\,\mat{V}^\tr =
    \mat{V}^\tr\mat{V} = \mat{I}$\pauseb
  \item And $\mat{S}$ is a $m\times n$ matrix with non-zero diagonal
    elements $S_{ii} = s_i$ known as the \emph{singular values}\pause
  \item Where did that come from?\pauseb
  \end{itemize}
\end{PauseHighLight}
...
\item But they are not normalised (since $\binom{\bm{u}_i}{\bm{v}_i}$
  ...
  \begin{align*}
    ...
    \mat{X}^\tr &= \tilde{\mat{V}} \,\mat{S}^\tr\, \tilde{\mat{U}}^\tr\pause
  \end{align*}
\end{itemize}
\end{PauseHighLight}
...
\begin{PauseHighLight}
  ...
  \item Now for any two matrices $|\mat{A}\,\mat{B}| =
    |\mat{A}|\,|\mat{B}|$\pause
  \item Thus $|\mat{M}| = |\mat{U}| \, |\mat{S}| \, |\mat{V}^\tr|\pause$
  \item For and orthogonal matrix $|\mat{U}| = \pm 1$\pause{} since
    $\mat{U}\,\mat{U}^\tr=\mat{I}\pauseb \Rightarrow
    |\mat{U}\,\mat{U}^\tr| = |\mat{I}|\pauseb \Rightarrow |\mat{U}|\,|\mat{U}^\tr| = 1$\pauseb{} or $|\mat{U}|^2 = 1$\pauseb 
  \item Thus
    \begin{align*}
