...
\begin{PauseHighLight}
  ...
  \item This would be a latent variable (it is not an observable or
    ...
    model)\pause
  \item This will be a random variable (it is uncertain, but we can
...
\begin{PauseHighLight}
  ...
  \item But it is not Bayesian\pause{} (despite what you are sometime
    told)\pauseb---its not properly probabilistic\pauseb
...
\begin{PauseHighLight}
  ...
  \item And, of course, doing a full probabilistic calculation has real
    advantages\pause{} (no overfitting, estimate of error, etc.)\pauseb
  \end{itemize}
\end{PauseHighLight}
