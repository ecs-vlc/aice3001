...
\begin{slide}
\section{The Goldern Rule of Mathematics}
...
\begin{PauseHighLight}
  ...
  \item Provided the matrices are the correct size we can compose
    ...
    \item and then applying $\mat{A}$ to get vectors in $\mathbb{R}^{\ell}$\pause
    \end{itemize}
    \begin{align*}
      \mat{A}\, \mat{B} \, \bm{x} = \mat{A} \,(\mat{B}\, \bm{x})
    \end{align*}
  \end{itemize}
\end{PauseHighLight}
...
\begin{PauseHighLight}
  ...
  \item The gradient of a function $f:\mathbb{R}^n
    \rightarrow \mathbb{R}^m$ at a point $\bm{x}^*$ is define as the
    column vector
    \begin{align*}
      \grad f(\bm{x}) =
      \begin{pmatrix}
...
\begin{PauseHighLight}
  ...
  \item If you don't, you are going to struggle following any machine
    learning couse, so grab a maths book and revise this\pause
