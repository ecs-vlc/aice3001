%% Standalone lecture -- build with:  pdflatex -shell-escape probability
\input{preamble}
  ...
  \item We attribute a \emph{probability}, $\Prob{\mathcal{E}}$, to an event,
    ...
    \begin{minipage}{0.6\linewidth}
      ...
      \item $\Prob{\mathcal{E}} + \Prob{\neg\mathcal{E}} = 1$ where
        ...
        \pauselevel{highlight =2 :3}\pause
      \end{itemize}
    \end{minipage}\hfill
...
\begin{PauseHighLight}
  ...
  \item It is useful to work with \emph{probability densities
    ...
    \begin{align*}
      ...
      \epsilon)}}{\vol{\mathcal{B}(\bm{x}, \epsilon)}} \hspace{2cm} \raisebox{-0.5cm}{\includegraphics[width=2cm]{infinessimal_ball}}
    \end{align*}
    where $\mathcal{B}(\bm{x}, \epsilon)$ is a ball of radius
    $\epsilon$ around the point $\bm{x}$ and
    $\vol{\mathcal{B}(\bm{x}, \epsilon)}$ is the volume of the ball\pause
  \item If we make a change of variable the volume
    $\vol{\mathcal{B}(\bm{x}, \epsilon)}$ might change so
...
\begin{PauseHighLight}
  ...
  \item Or
    \begin{align*}
      ...
      \pause
    \end{align*}
  \item A probability density measured in units of probability per
