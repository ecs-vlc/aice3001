...
\begin{slide}
\section[-2]{Complex Likelihoods}

\begin{PauseHighLight}\squeeze
  \begin{itemize}
  \item In the classical problems with parameters,
    $\bm{\theta}$, that are well known statistics such as the mean and
    variance\pause
  \item But suppose we are trying to distinguish between different
    models of cosmological inflation (physics models of the early
    Universe)\pause
  \item Our data is the Cosmic Microwave Background measure by a
    satellite telescope\pause
  \item Our parameter, $\bm{\theta}$, might represent unknown
    parameters of the early universe (width of surface of scattering,
    number of e-foldings, primordial perturbations, amount of
    cool dark matter)\pause
  \item Here the likelihood $\Prob{\text{CMB} \mid \bm{\theta}}$ is
    far from a well known probability distribution\pause
  \end{itemize}
\end{PauseHighLight}
...
\begin{PauseHighLight}
  ...
  \item These techniques have been highly developed, but very frequently
    ...
    inference\pause
  \item MCMC is very easy to use, although it can be slow\pause
  \item There exist other methods such as variational approximations
    that can be much quicker, but are approximations\pause
  \end{itemize}
\end{PauseHighLight}
...
\begin{PauseHighLight}
  \begin{itemize}
  \item How does the transformation method work to generate random
    deviates?
    \begin{enumerate}\setlength\itemsep{0.5em}
    \item You transform from one distribution to another
    \item You generate a uniformly distributed random number and apply
      the inverse probability distribution function to it
    \item You generate a uniformly distributed random number and apply
      the inverse cumulative distribution function to it
    \item You generate a uniformly distributed random number and apply
      the probability distribution function to it\pause
    \end{enumerate}
  \item Answer: 3. You generate a uniformly distributed random number
    and apply the inverse cumulative distribution function to
    it\pauseb
  \end{itemize}
\end{PauseHighLight}
\end{slide}
...
\begin{PauseHighLight}
  \begin{itemize}
  \item How does the rejection method work to generate random
    deviates?
    \begin{enumerate}\setlength\itemsep{0.3em}
    \item You generate from a nearby distribution and apply a
      transformation to the actual distribution
    \item You generate a random deviate, $Y$ with a distribution $g_Y$
      such that $\forall x,\;c\, g_Y(x) \geq f_X(x)$ and then choose a uniform
      deviate $U$ and accept $Y$ if $U\,c\,g_Y(Y) \leq f_X(Y)$
    \item You generate a random deviate, $Y$ with a distribution $g_Y$
      such that $\forall x,\;c\, g_Y(x) \leq f_X(x)$ and then choose a uniform
      deviate $U$ and accept $Y$ if $U\,c\,g_Y(Y) \geq f_X(Y)$\pause
    \end{enumerate}
  \item Answer: 2. You generate a random deviate, $Y$ with a
    distribution $g_Y$ such that $c\, g_Y(x) \geq f_X(x)$ and then
    choose a uniform deviate $U$ and accept $Y$ if
    $U\,c\,g_Y(Y) \leq f_X(Y)$\pauseb
  \end{itemize}
\end{PauseHighLight}
\end{slide}
...
\begin{PauseHighLight}
  \begin{itemize}
  \item What is detailed balance?
    \begin{enumerate}\setlength\itemsep{0.5em}
    \item It means you visit all possible values of the random
      variable equally often
    \item It is a Markov Chain that converges to a particular
      distribution
    \item It is the condition that the transition matrix is stochastic
    \item It is the equation $M_{ij} \, \pi_j = M_{ji} \pi_i$\pause
    \end{enumerate}
  \item Answer: 4. It is the equation
    $M_{ij} \, \pi_j = M_{ji} \pi_i$\pauseb
  \end{itemize}
\end{PauseHighLight}
\end{slide}
...
\begin{PauseHighLight}
  \begin{itemize}
  \item What statement is \emph{false} about the transition matrix
    $M_{ij}$?
    \begin{enumerate}\setlength\itemsep{0.5em}
    \item Each column sums to one, i.e. it is a stochastic matrix
    \item The absolute size of the eigenvalues are all less than or
      equal to~1
    \item Both its rows and columns sum to one, i.e. it is doubly
      stochastic
    \item The equilibrium state correspond to the eigenvectors with
      eigenvalue 1\pause
    \end{enumerate}
  \item Answer: 3. Both its rows and columns sum to one, i.e. it is
    doubly stochastic---\emph{this is not true}\pauseb
  \end{itemize}
\end{PauseHighLight}
\end{slide}
...
\begin{PauseHighLight}
  \begin{itemize}
  \item What is the difference between the Metropolis algorithm
    and Metropolis-Hastings
    \begin{enumerate}\setlength\itemsep{0.3em}
    \item They are the same algorithm
    \item In Metropolis-Hastings you chose the proposal move from a
      non-symmetric distribution which you compensate for in the
      acceptance criteria
    \item You choose a transition probability that does not satisfy
      detailed balance
    \item You perform a molecular dynamics simulation to obtain the
      proposal move\pause
    \end{enumerate}
  \item Answer: 2.In Metropolis-Hastings you chose the proposal move
    from a non-symmetric distribution which you compensate for in the
    acceptance criteria \pauseb
  \end{itemize}
\end{PauseHighLight}
\end{slide}
...
\begin{PauseHighLight}
  \begin{itemize}
  \item What statement is \textit{false} about MCMC
    \begin{enumerate}\setlength\itemsep{0.3em}
    \item Once you have reached an equilibrium state every state after
      that is an independent deviate
    \item It is useful for Bayesian inference because you don't have
      to compute the marginal likelihood
    \item Generally it is difficult to determine if you have reached
      an equilibrium state
    \item To compute expectation you can use every state once you have
      reached equilibrium, but you need to count states if they are
      accepted or rejected, as well as run for a long time\pause
    \end{enumerate}
  \item Answer: 1. Once you have reached an equilibrium state every state after
      that is an independent deviate---this is \emph{false}\pauseb
  \end{itemize}
\end{PauseHighLight}
\end{slide}
