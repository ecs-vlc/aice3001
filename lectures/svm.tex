%% Standalone lecture -- build with:  pdflatex -shell-escape svm
\input{preamble}
...
\begin{PauseHighLight}
  ...
  \item It is a \emph{strongly convex} optimisation problem so will
    have a unique minimum\pause
...
\begin{PauseHighLight}
  ...
  \item In the extended feature space we can find a separating
    plane (given by $\bm{w}$ and $b$) with maximum margin by solving
    the problem
   \begin{align*}
...
\begin{PauseHighLight}
  ...
  \item Differentiating the Lagrangian with respect to $\bm{w}$
    ...
    \end{align*}
  \item $\gradw \mathcal{L}=\pauselevel{build =2, highlight =2} \pause\bm{w} -\sum\limits_{k=1}^m
    \alpha_k\,y_k\,\bm{\phi}(\bm{x}_k)\pauselevel{build =2, highlight =2 :3}\pause= 0\pauseb$ $\implies$ $\bm{w}^* = \sum\limits_{k=1}^m
  \alpha_k\, y_k\, \bm{\phi}(\bm{x}_k)$\pauseb
  \item $\pd{\mathcal{L}}{b}=\pauselevel{build =6, highlight =6}\pause
    \sum_{k=1}^m \alpha_k\,
    y_k\pauselevel{build =6, highlight  =6 :7}\pause=0$\pauseb
  \item Substituting back into the Lagrangian
   \begin{align*}
     \max_{\bm{\alpha}\geq\bm{0}}\mathcal{L}(\bm{w}^*, b^*, \bm{\alpha})
     = \max_{\bm{\alpha}\geq\bm{0}} \,\, \sum_{k=1}^m \alpha_k 
      - \frac{1}{2} \sum_{k,l=1}^m \alpha_k\,\alpha_l
      \,y_k\,y_l\, \inner{\bm{\phi}(\bm{x}_k)}{\bm{\phi}(\bm{x}_l)} \pauseb
    \end{align*}    
  \end{itemize}
\end{PauseHighLight}
...
\begin{slide}
\section[-1]{Choosing the Right Kernel Function}
...
\begin{PauseHighLight}
  ...
  \item Kernels often come with parameters, e.g. the popular radial
    ...
    \begin{align*}
       K(\bm{x},\,\bm{y}) =
      \mathrm{e}^{-\gamma\,\|\bm{x}-\bm{y}\|^2}\pause \hspace{3cm} \raisebox{-1cm}{\includegraphics[height=3cm]{rbf_kernel}}
    \end{align*}
