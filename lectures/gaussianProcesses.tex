%% Standalone lecture -- build with:  pdflatex -shell-escape gaussianProcesses
\input{preamble}
...
\begin{minipage}{10cm}\raggedright
  \begin{enumerate}\squeeze
    \outlineitem{ML with Gaussians}{mlgausee}
...
\begin{slide}
\section[-1]{Machine Learning with Gaussians}

\begin{PauseHighLight}
  \begin{itemize}
  \item Due to the fact that we can analytically integrate over
    multivariate Gaussians they are heavily used of machine
    learning\pause
  \item Examples include multivariate modelling, Kalman filters and
    Gaussian Processes\pause
  \item Despite being analytically tractable, they are very fiddly\pause
  \item This is mainly because it is easy to specify means,
    $\bm{\mu}$, and covariance, $\mat{C}$, but Gaussians involve the
    inverse covariance matrix
    \begin{align*}
      \mathcal{N}\left(\bm{x} \mid \bm{\mu}, \mat{C}\right) =
      \frac{\e{-\tfrac{1}{2} (\bm{x} - \bm{\mu})^\tr \mat{C}^{-1}
      (\bm{x}-\bm{\mu})}}{\sqrt{\det(2\,\pi\,\mat{C})}}\pause
    \end{align*}
  \end{itemize}
\end{PauseHighLight}
...
\begin{slide}
\section[-1.5]{Completing the Square}

\begin{PauseHighLight}
  \begin{itemize}
  \item One of the key tools for performing Gaussian integrals
    is ``completing the square''
    \begin{align*}
      -\frac{1}{2 }\bm{x}^\tr \mat{C}^{-1}\,\bm{x} + \bm{a}^\tr \bm{x}
      &= -\frac{1}{2} \bm{x}^\tr \mat{C}^{-1}\,\bm{x} + \frac{1}{2}
        \bm{a}^\tr \bm{x} + \frac{1}{2} \bm{x}^\tr \bm{a} \pause \\ 
      &= -\frac{1}{2} \bm{x}^\tr \mat{C}^{-1}\,\bm{x} + \frac{1}{2} \bm{a}^\tr
        \mat{C} \mat{C}^{-1} \bm{x} +\frac{1}{2} \bm{x}^\tr
        \mat{C}^{-1} \mat{C} \bm{a} \pauseb \\
      &= -\frac{1}{2} (\bm{x}^\tr - \bm{a}^\tr\mat{C}) \mat{C}^{-1}
        (\bm{x}-\mat{C} \bm{a}) + \frac{1}{2} \bm{a}^\tr \mat{C}\,\bm{a}\pauseb
    \end{align*}
  \item This gives us the Hubbard–Stratonovich transformation
    \begin{align*}
      \int_{-\infty}^{\infty} 
      \tfrac{\e{-\frac{1}{2 }\bm{x}^\tr \mat{C}^{-1}\,\bm{x} + \mat{a}^\tr
      \mat{x}}}{\sqrt{|\det(2\,\pi\,\mat{C})|}} \, {\scriptstyle \dd \bm{x}}
      &= \int_{-\infty}^{\infty}
      \tfrac{\e{-\frac{1}{2} (\bm{x}^\tr - \bm{a}^\tr\mat{C}) \mat{C}^{-1}
        (\bm{x}-\mat{C} \bm{a}) + \frac{1}{2} \bm{a}^\tr
        \mat{C}\,\bm{a}}}{\sqrt{|\det(2\,\pi\,\mat{C})|}} \, {\scriptstyle \dd \bm{x}}\pauseb \\
      &= \int_{-\infty}^{\infty} \mathcal{N}\left(\bm{x}\mid
        \mat{C}\,\bm{a}, \mat{C}\right) \,
        \e{\frac{1}{2} \bm{a}^\tr \mat{C}\,\bm{a}} \dd \bm{x}\pauseb
        = \e{\frac{1}{2} \bm{a}^\tr \mat{C}\,\bm{a}} \pauseb
    \end{align*}
  \end{itemize}
\end{PauseHighLight}
...
\begin{PauseHighLight}
  \begin{itemize}
  \item Again through completing the square we can show
    \begin{align*}
      M(\bm{a}) = \int_{-\infty}^{\infty} \e{\bm{a}^\tr \bm{x}} \,
      \mathcal{N}(\bm{x}\mid \bm{\mu}, \mat{C}) \, \dd \bm{x}
      = \e{\bm{a}^\tr \bm{\mu} + \frac{1}{2} \bm{a}^\tr \mat{C} \bm{a}}\pause
    \end{align*}
  \item The first moment is given by
    \begin{align*}
      \av{\bm{x}} = \left. \grad_{\bm{a}} M(\bm{a}) \right|_{\bm{a}=0}
      \pause = \left. (\bm{\mu} + \mat{C}
      \bm{a})\e{\bm{a}^\tr \bm{\mu} + \frac{1}{2} \bm{a}^\tr \mat{C}
      \bm{a}} \right|_{\bm{a}=0} \pause =
      \bm{\mu}\pause
    \end{align*}
  \item The second moment is
    \begin{align*}
      \av{x_i\,x_j} = \left. \frac{\partial^2 M(\bm{a})}{\partial
      x_i\,\partial x_j}  \right|_{\bm{a}=0} \pause= \mu_i \mu_j + C_{ij}\pause
    \end{align*}
  \end{itemize}
\end{PauseHighLight}
...
\begin{PauseHighLight}
  \begin{itemize}
  \item The covariance is given by
    \begin{align*}
      \av{(x_i-\mu_i)\,(x_j-\mu_j)} = C_{ij}\pause
    \end{align*}
    or $\av{(\bm{x}-\bm{\mu})(\bm{x}-\bm{\mu})^\tr} = \mat{C}$\pauseb
  \item The cumulant generating function is given by
    \begin{align*}
      C(\bm{a}) = \log(M(\bm{a}))
      = \bm{a}^\tr \bm{\mu} + \frac{1}{2} \bm{a}^\tr \mat{C} \bm{a}\pause
    \end{align*}
  \item The mean is equal to $\grad C(\bm{a}) |_{\bm{a}=0} =
    \bm{\mu}$\pause
  \item The covariance is given by $\left. \frac{\partial^2
        C(\bm{a})}{\partial a_i \,\partial a_j} \right|_{\bm{a}=0} =
    C_{ij}$\pause
  \item Normal distributions have no high cumulants (skewness, curtosis)\pause
  \end{itemize}
\end{PauseHighLight}
...
\end{center}
\begin{align*}
  \mathcal{N} \!\left( \begin{psmallmatrix}\bm{x}\\ \bm{y}\end{psmallmatrix} \,\middle|\, \begin{psmallmatrix} \bm{\mu}^{x} \\ \bm{\mu}^y \end{psmallmatrix}, \begin{psmallmatrix} \mat{A} & \mat{B}\\ \mat{B}^{\tr} & \mat{C} \end{psmallmatrix} \right)
  = \frac{\e{-\frac{1}{2}
  (\bm{x}^\tr-\bm{\mu}^{x\tr},  \bm{y}^\tr-\bm{\mu}^{y\tr})
  \begin{psmallmatrix} \mat{A} & \mat{B}\\
    \mat{B}^{\tr} & \mat{C} \end{psmallmatrix}^{-1}
  \begin{psmallmatrix}\bm{x}-\bm{\mu}^x\\ \bm{y}-\bm{\mu}^y\end{psmallmatrix}}}
  {\sqrt{\det\left(2\,\pi \begin{psmallmatrix} \mat{A} & \mat{B}\\
    \mat{B}^{\tr} & \mat{C} \end{psmallmatrix}\right)}} \pauseh
\end{align*}
...
\begin{slide}
\section{Marinalising Multivariate Normals}

\begin{PauseHighLight}
  \begin{itemize}
  \item I can define a moment generating function,
    \begin{align*}
      M(\bm{a},\bm{b}) &= \int \e{(\bm{a}^\tr, \bm{b}^\tr)
      \begin{psmallmatrix}
        \bm{x}-\bm{\mu}^x\\ \bm{y}-\bm{\mu}^y
      \end{psmallmatrix}}
       \, \mathcal{N} \!\left( \begin{psmallmatrix}\bm{x}\\
        \bm{y}\end{psmallmatrix} \,\middle|\, \begin{psmallmatrix} \bm{\mu}^{x}
          \\ \bm{\mu}^y \end{psmallmatrix}, \begin{psmallmatrix} \mat{A}
            & \mat{B}\\ \mat{B}^{\tr} & \mat{C} \end{psmallmatrix} \right)
                         \dd \bm{x} \, \dd \bm{y} \pause\\
        &= \e{\frac{1}{2} (\bm{a}^\tr, \bm{b}^\tr) \begin{psmallmatrix} \mat{A}
            & \mat{B}\\ \mat{B}^{\tr} & \mat{C} \end{psmallmatrix} \begin{psmallmatrix}
        \bm{a}\\ \bm{b}
            \end{psmallmatrix}}\pause
          = \e{\frac{1}{2}\left(\bm{a}^\tr \mat{A}\bm{a} + \bm{b}^\tr
          \mat{C}\bm{b} +2\bm{a}^\tr \mat{B}\bm{b} 
          \right)}\pause
    \end{align*}
\item If I set $\bm{b}=\bm{0}$ this is equivalent to marginalising out
  $\bm{y}$\pause
\item But then we just get $M(\bm{a},\bm{0}) =
  \e{\frac{1}{2}\bm{a}^\tr \mat{A}\bm{a}}$\pause
\item This is the same as the moment generating function for
  $\mathcal{N}(\bm{x} \mid \bm{\mu}^x, \mat{A})$\pause
  \end{itemize}
\end{PauseHighLight}
...
\begin{slide}
\section{Marinalising Multivariate Normals Continued}

\begin{PauseHighLight}
  \begin{itemize}
  \item An important and non-trivial result is
    \begin{align*}
      \int \mathcal{N} \!\left( \begin{psmallmatrix}\bm{x}\\
        \bm{y}\end{psmallmatrix} \,\middle|\, \begin{psmallmatrix} \bm{\mu}^{x}
          \\ \bm{\mu}^y \end{psmallmatrix}, \begin{psmallmatrix} \mat{A}
            & \mat{B}\\ \mat{B}^{\tr} & \mat{C} \end{psmallmatrix} \right)
      \dd \bm{y}
      =  \mathcal{N}(\bm{x} \mid \bm{\mu}^x, \mat{A})\pause
    \end{align*}
  \item You would think this is easy to prove by straightforward
    integration\pause, but it ain't\pauseb
  \item It can be done, but it is a page of work\pause
  \end{itemize}
\end{PauseHighLight}
...
\begin{PauseHighLight}
  ...
  \item This is typical of many Bayesian problems where we have some set
