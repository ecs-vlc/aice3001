%% Standalone lecture -- build with:  pdflatex -shell-escape bayes
\input{preamble}
...
\begin{PauseHighLight}
  ...
  \item However, this requires that we encode our prior knowledge of the
    problem and specify a likelihood\pause
...
\begin{PauseHighLight}
  ...
  \item The normalisation term 
    ...
    \item At one level this is just a normalisation condition\pause,
      which is often a pain to compute\pauseb
    \item But it also plays an important role in \emph{model selection}\pause
  \end{itemize}
\end{PauseHighLight}
...
\begin{PauseHighLight}
  \begin{itemize}
  \item Suppose we have two models of the likelihood or prior; $M_1$
    and $M_2$. Then the posterior will depend on the model
    \begin{align*}
      \Prob{\mathcal{H}\mid \data, M_i} = \frac{\Prob{\data \mid
      \mathcal{H}, M_i}\, \Prob{\mathcal{H}\mid M_i}}{\Prob{\data\mid
      M_i}}\pause 
    \end{align*}
  \item We can use $\Prob{\data\mid M_1}$ or $\Prob{\data\mid M_2}$ to
    select which model is most likely!\pause
  \item E.g. is my likelihood best captured by a normal or gamma
    distribution\pause
...
\begin{PauseHighLight}
  ...
  \item The expected value of $p$ is given by $a/(a+b) = 23/32 = 0.72$\pause
  \item The standard deviation is
    \begin{align*}
      \sqrt{\frac{a\,b}{(a+b)^2(a+b+1)}} = 0.08\pause
    \end{align*}
  \end{itemize}
\end{PauseHighLight}
...
\begin{align*}
  ...
  \sqrt{\Var{\mu}} &=  \sqrt{\frac{a}{b^2}} = 0.5\pause
\end{align*}
\end{slide}
