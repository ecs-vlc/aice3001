%% Standalone lecture -- build with:  pdflatex -shell-escape vectorSpaces
\input{preamble}
...
\begin{PauseHighLight}
  \begin{itemize}
  \item Any set of object that satisfies the axioms of a vector spacer
    are \textit{vectors}\pause---not just $\bm{v}\in \mathbb{R}^n$\pauseb
  ...
  \item Functions form a vector space\pause
    ...
    \end{itemize}
  \item Bounded vectors in $\mathbb{R}^n$ \emph{don't} form a vector space\pause
  \end{itemize}
\end{PauseHighLight}
...
\begin{PauseHighLight}
  ...
  \item We say $d(\bm{x}, \bm{y})$ is a \emph{distance} or \emph{metric} if
    \begin{enumerate}\squeeze
...
\begin{PauseHighLight}
  \begin{itemize}
  \item One way to characterise a well behaved function, $f(x)$, is if there
    exists a number $K<\infty$ such that for all $x$ and $y$
    \begin{align*}
      d(f(x), f(y)) \leq K\, d(x,y)\pause
    \end{align*}
  \item This is known as a \emph{Lipschitz condition} and the function is
    said to be $K$-Lipschitz\pause{} or Lipschitz continuous\pauseb
...
\begin{slide}
\section[-1.5]{Lipschitz Continuous Functions}
  
\pb\pause\pauselevel{=1}
\begin{center}
  \multipdf[width=\linewidth]{discontinuous_func}\pause
\end{center}
\begin{itemize}
\item Definition is applicable to any function defined in a
  metric space\pause
\end{itemize}
\end{slide}
...
\begin{PauseHighLight}
  ...
  \item To formalise this we define the \emph{norm} of an object
    ...
    \begin{enumerate}
    \item $\| \bm{v} \| >0$ if $\bm{v}\neq\bm{0}$ and $\|\bm{0}\|=0 $\hfill(non-negativity)\pause
...
\begin{PauseHighLight}
  ...
  \item Some, but not all, norms satisfy the inequality
    \begin{align*}
...
\begin{PauseHighLight}
  \begin{itemize}
  \item Deep learning involves multiplying the input (which we can think
...
\begin{PauseHighLight}
  ...
  \item Functions can also have norms, for example, if $f(x)$ is defined
    ...
    \begin{align*}
      \| f \|_{L^2} = \sqrt{\int_{x\in\mathcal{I}} f^2(x) \, \dd x}\pause
    \end{align*}}
  \item The $L^2$ vector space is the set of function where $\|
    f \|_{L^2}<\infty$\pause
  \item The $L^1$-norm is given by
    $\| f \|_{L^1} = \int_{x\in\mathcal{I}} |f(x)| \, \dd x$\pause
...
\begin{PauseHighLight}
  \begin{itemize}
  \item Given a norm, there exists an induced distance
    \begin{align*}
      d(\bm{x}, \bm{y}) = \| \bm{x} - \bm{y} \|\pause
    \end{align*}
  \item Any normed space has an associated metric space\pause
  \item Thus we can define the Frobenious-distance between matrices
    \begin{align*}
      d_F(\mat{A}, \mat{B}) = \| \mat{A} - \mat{B} \|_F\pause
    \end{align*}
  \item It is easy to see that non-negativity, linearity and
    triangular inequality for distances are all inherited from the
    same property for norms\pause
  \end{itemize}
\end{PauseHighLight}
