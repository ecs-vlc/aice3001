%% Standalone lecture -- build with:  pdflatex -shell-escape boosting
\input{preamble}
...
\begin{PauseHighLight}
  \begin{itemize}
  \item Boosting is another \textit{ensemble learning method\/} that make a
    \emph{strong learner} by using a weighted sum of \emph{weak
      learners}
    \begin{align*}
...
\begin{PauseHighLight}
  ...
  \item At each step $\hat{h}_n(\bm{x})$ is trained to predict the
    \emph{residual error}, $\Delta_{n-1} = y-C_{n-1}(\bm{x})$,
    (i.e. the target minus the current prediction)\pause
...
\begin{slide}
\section{Ensembling in Deep Learning}

\begin{PauseHighLight}
  \begin{itemize}
  \item For most machine learning ensembling different machines
    usually gives a reasonable improvement in performance\pause
  \item The machines should have roughly the same performance\pause
  \item Of course, this comes at the price of having to train multiple
    machines\pause
  \item One can try to train a machine to decide how to combine
    different machines (stacking)\pause, but beware, it is very easy
    to overfit\pauseb
  \item Usually better to average predictions for regression or do
    majority voting for classification problems\pause
  \end{itemize}
\end{PauseHighLight}
...
\pb
\begin{itemize}
\item For deep learning we can control for over-fitting using
  dropout\pause
  \begin{center}
    \multipdf{dropOut}\pause
  \end{center}
\item This can be seen as ensembling lots of much simpler machines\pause
\end{itemize}
