%% Standalone lecture -- build with:  pdflatex -shell-escape integration
\input{preamble}
...
\begin{PauseHighLight}
  ...
  \item Let
    ...
    \end{align*}
  \item Now for small $\delta x$ \hspace{12cm}
    \makebox(0,0){\includegraphics[width=8cm]{fundamental_law}}
    \vspace{1cm}
    \begin{align*}
      I(a,x+\delta x) =  \int_a^{x+\delta x} f(z) \, \mathrm{d}z\pause
      =  \lim\limits_{\delta z_i\rightarrow 0} \sum_{i=0}^{n-1}
      (z_{i+1} - z_i) \, f(z_i) + \delta x \, f(x)\pauseb
    \end{align*}
  \item Thus
    \begin{align*}
      \frac{\dd I(a, x)}{\dd x}
...
\begin{PauseHighLight}
  ...
  \item We can define a momentum generating function
    ...
    \end{align*}
  \item Then $M_n = Z^{(n)}(0)$\pause{} (the $n^{th}$ derivative of $Z(\ell)$
    at $\ell = 0$)\pauseb
    \begin{align*}
