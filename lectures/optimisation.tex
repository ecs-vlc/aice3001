%% Standalone lecture -- build with:  pdflatex -shell-escape optimisation
\input{preamble}
...
\item To minimise we $L(\bm{w}|\data)$ we compute the gradient $\grad
  L(\bm{w}|\data)$\pause 
\item For neural networks an efficient algorithm for computing the
  gradient is known as back-propagation of errors (\textit{back-prop})\pause
\end{itemize}
...
\item If we are at a distance $x-x^*=\epsilon$ from the minima then
  after one iteration we will be a distance $\epsilon^2$\pause, after
  two iterations we will be at a distance $\epsilon^4$, etc.\pauseb
