...
\begin{slide}
\section[-2]{Chain Rule in High Dimensions}
...
\begin{PauseHighLight}
  ...
  \item Since
    \begin{align*}
      ...
      \bm{u}^\tr \grad_{\bm{x}} h(\bm{x}) + O(\epsilon^2)\pause
    \end{align*}
    and (recall $h(\bm{x}) = f(\bm{g}(\bm{x}))$)
    \begin{align*}
      h(\bm{x}+\epsilon \, \bm{u}) = h(\bm{x}) + \epsilon\,  \, 
  ...
  \item Componentwise
    \begin{align*}
      ...
      \sum_{j=1}^m \frac{\partial f(\bm{g}(\bm{x}))}{\partial g_j}
      \frac{\partial g_j(\bm{x})}{\partial x_i} \pause
    \end{align*}
