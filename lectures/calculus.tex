...
\begin{minipage}{10cm}\raggedright
  \begin{enumerate}\squeeze
    ...
    \outlineitem{Vector Calculus}{vector}
    \outlineitem{Matrix Calculus}{vector}
  \end{enumerate}
\end{minipage}\hfill
\begin{minipage}{12cm}
...
\begin{PauseHighLight}
  \begin{itemize}
    \item Consider the derivative of $a\,f(x)+b\,g(x)$
    \begin{align*}
      ...
      &=
      \lim_{\epsilon\rightarrow 0}
      \frac{a\, \bra{f(x+\epsilon)-f(x)} + b\,\bra{g(x+\epsilon)-g(x)}}{\epsilon}
      \pauseb \\
      &=  \lim_{\epsilon\rightarrow 0}
        a\, \bra{\frac{f(x+\epsilon)-f(x)}{\epsilon}} +
        b\,\bra{\frac{g(x+\epsilon)-g(x)}{\epsilon}}\pauseb \\
      &= a\,f'(x) + b\,g'(x) \pauseb
    \end{align*}
...
\begin{PauseHighLight}
  ...
  \item To compute the derivative in a direction
    ...
    \end{align*}
  \item If $\bm{u} = \bm{\delta}_i = (0,\ldots,0,1,0,\ldots,0)^\tr$
    (i.e. $u_i = 1$) then
    \begin{align*}
...
\begin{PauseHighLight}
  \begin{itemize}
  \item The gradient tells us about the change in a function value
    when we perturb an input $\bm{x}$ by a small amount in the
    directions $\bm{u}$
    \begin{align*}
      f(\bm{x}+\epsilon \, \bm{u}) = f(\bm{x}) + \epsilon\,
      \bm{u}^\tr \grad f(\bm{x}) + O(\epsilon^2)\pause
    \end{align*}
  \item What happens when we compose functions: $h(\bm{x}) =
    f(\bm{g}(\bm{x}))$\pause.  Note
    that,
    \begin{align*}
      x&\in \mathbb{R}^n
      & \bm{g}:\mathbb{R}^n & \rightarrow \mathbb{R}^m
      & f:\mathbb{R}^m & \rightarrow \mathbb{R}
      & h:\mathbb{R}^n & \rightarrow \mathbb{R}\pause
    \end{align*}
  \item The vector valued function $\bm{g}(\bm{x})$ can be seen as $m$
    independent function $
      \bm{g}(\bm{x}) =
      \begin{pmatrix}
        g_1(\bm{x}) \\ g_2(\bm{x}) \\ \vdots \\ g_m(\bm{x})
      \end{pmatrix}\pause$
  \end{itemize}
\end{PauseHighLight}
\end{slide}
...
\begin{PauseHighLight}
  \begin{itemize}
  \item Perturbing $\bm{x}$ by $\epsilon\, \bm{u}$
    \begin{align*}
      g_i(\bm{x}+\epsilon \, \bm{u}) = g_i(\bm{x}) + \epsilon\,
      \bm{u}^\tr \grad g_i(\bm{x}) + O(\epsilon^2)\pause
    \end{align*}
    so that
    \begin{align*}
      \bm{g}(\bm{x}+\epsilon \, \bm{u}) = \bm{g}(\bm{x}) +
      \epsilon\, \mat{J} \,\bm{u}  + O(\epsilon^2)
    \end{align*}
    where $\mat{J}$ is an $m\times n$ matrix with components $J_{ij}=
    \frac{\partial g_i(\bm{x})}{\partial x_j}$\pause
  \item Note that $\mat{J}\, \bm{u}$ is a column vector of size $m$\pause
  \item Ignoring terms of order $\epsilon^2$
    \begin{align*}
      h(\bm{x}+\epsilon \, \bm{u}) &= f\bra{\bm{g}(\bm{x} +
                                       \epsilon \, \bm{u})}\pause
      = f\bra{\bm{g}(\bm{x}) + \epsilon \mat{J}\, \bm{u}}\pauseb \\
      &= f\bra{\bm{g}(\bm{x})} + \epsilon\, \bm{u}^\tr \mat{J}^\tr \, \grad_{\bm{g}}
        f\bra{\bm{g}(\bm{x})} \pauseb
    \end{align*}
  \end{itemize}
\end{PauseHighLight}
...
\begin{slide}
\section[-1]{Chain Rule in High Dimensions}

\begin{PauseHighLight}
  \begin{itemize}
  \item Using $h(\bm{x}) = f(\bm{g}(\bm{x}))$
    \begin{align*}
      h(\bm{x}+\epsilon \, \bm{u}) = h(\bm{x}) + \epsilon\,  \, 
        \bm{u}^\tr \mat{J}^\tr \grad_{\bm{g}} f\bra{\bm{g}(\bm{x})} \pause
    \end{align*}
  \item We get that
    \begin{align*}
      \grad_{\bm{x}} h(\bm{x}) =  \grad_{\bm{x}} f\bra{\bm{g}(\bm{x})} =  \mat{J}^\tr \grad_{\bm{g}} f\bra{\bm{g}(\bm{x})} \pause
    \end{align*}
  \item Componentwise
    \begin{align*}
      \frac{\partial h(\bm{x})}{\partial x_i} =
      \frac{\partial f\bra{\bm{g}(\bm{x})}}{\partial x_i}\pause =
      \sum_{j=1}^m \frac{\partial g_j(\bm{x})}{\partial x_i}
      \frac{\partial f(\bm{g}(\bm{x}))}{\partial g_j} \pause
    \end{align*}
...
\begin{PauseHighLight}\squeeze
  ...
  \item We note that
    \begin{align*}
      \mathrm{tr}\, \mat{A} \,\mat{B} = \sum_{i,j} A_{ij} B_{ji} \pause=
      \sum_{i,j} B_{ij} A_{ji}\pauseb =  \mathrm{tr} \mat{B} \, \mat{A}\pauseb
    \end{align*}
...
\begin{PauseHighLight}
  ...
  \item E.g.
    \begin{align*}
      ...
      &=  \lim_{\epsilon\rightarrow0} \; \frac{1}{\epsilon} \; \bra{ \mathrm{tr}\;
...
\begin{PauseHighLight}
  ...
  \item When we look at \emph{integration} it gets harder\pauseb
  \end{itemize}
\end{PauseHighLight}
