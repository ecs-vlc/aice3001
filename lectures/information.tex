%% Standalone lecture -- build with:  pdflatex -shell-escape information
\input{preamble}
...
\begin{PauseHighLight}
  ...
  \item An upper bound on the number of code words we can send in a
    ...
    \begin{align*}
      \frac{2^n}{|B(\bm{x}_i, r)|} = \frac{2^n}{\binom{n}{r}} \pause
      = 2^n\, \e{- \log(n!) + \log(r!) + \log((n-r)!)} \pauseb
      = c\,\sqrt{n} \, 2^{I_{X;Y}}\pauseb
    \end{align*}
  \end{itemize}
\end{PauseHighLight}
\end{slide}
...
\begin{PauseHighLight}
  ...
  \item  Thus $-\av[X\sim p(X)]{\logg{q(X)}}$ is the expected length of
    ...
    distribution $q(X)$ rather than $p(X)$\pause
...
\begin{PauseHighLight}
  ...
  \item We can rewrite the variational free energy as $\Phi(\bm{\phi})
    =-\int g(\bm{\theta}\mid\bm{\phi})
      \logg{\frac{g(\bm{\theta}\mid\bm{\phi})}{f(\bm{\theta}, \data)}} \,
      \dd \bm{\theta}\pause =  L_g(\bm{\phi}) + H_g(\bm{\phi})$\pause{} where
    \begin{align*}
      L_g(\bm{\phi}) = \int g(\bm{\theta}\mid\bm{\phi}) \bra{
      \logg{\strut f(\data| \bm{\theta}) \, f(\bm{\theta})} } \, \dd \bm{\phi}
    \end{align*}
    acts like an expected posterior that is maximised when the
    data is well modelled (we put
    $g(\bm{\theta}\mid\bm{\phi})$ where $f(\bm{\theta}, \data)$ is
    large, c.f. MAP)\pause
  \item The second term is an entropy
    \begin{align*}
      H_g(\bm{\phi}) = - \int g(\bm{\theta}\mid\bm{\phi})
...
\begin{PauseHighLight}
  ...
  \item Variational methods can be much faster than MCMC (although
    they tend to involve some iterations to maximise the variation
    free energy)\pause
  \item They can produce very good approximations, although this is not
...
\begin{PauseHighLight}
  ...
  \item The \emph{description length} for $\{y_i \mid i=1,2,\ldots,m\}$ is
    ...
    $\theta_k$ (we get to choose the accuracy it is worth encoding the
    parameters)\pause
